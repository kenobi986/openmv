...
while True:
    ...
    try:
        ta, ir, to_min, to_max, t_14 = fir.read_ir()
    except OSError:
        continue

    if not ALT_OVERLAY:
        # Scale the image and blend it with the framebuffer
    ...
    # Print t_14 data for debugging
    print(t_14)

    # Create an image object to store the 14-bit data
    t14_img = image.Image(sensor.width(), sensor.height(), sensor.GRAYSCALE)

    # Populate the image with the t_14 data
    for y in range(sensor.height()):
        for x in range(sensor.width()):
            # Scale 14-bit data to 16-bit range (0-65535) for BMP
            value = t_14[y * sensor.width() + x] << 2  # Shift left by 2 to fit into 16-bit
            t14_img.set_pixel(x, y, value)

    # Save the image as BMP file to the SD card
    t14_img.save("/t14_image.bmp")
